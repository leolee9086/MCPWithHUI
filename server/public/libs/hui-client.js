...
// ../../node_modules/.pnpm/@modelcontextprotocol+sdk@1.11.0/node_modules/@modelcontextprotocol/sdk/dist/esm/types.js
...
// ../../node_modules/.pnpm/@modelcontextprotocol+sdk@1.11.0/node_modules/@modelcontextprotocol/sdk/dist/esm/shared/protocol.js
...
// ../../node_modules/.pnpm/@modelcontextprotocol+sdk@1.11.0/node_modules/@modelcontextprotocol/sdk/dist/esm/client/index.js
...
// src/client/HuiMcpClient.ts
var SingleToolSchemaForClient = ListToolsResultSchema.shape.tools.element;
var HUI_META_TOOL_NAME = "listHuiDefinitions";
var HuiDefinitionsResultSchema = z.object({
  tools: z.array(z.object({
    name: z.string(),
    huiHints: z.any()
  }))
});
var GenericJsonDataContentSchema = z.object({
  type: z.string(),
  // 宽松匹配 type，实际可能是 'json' 或其他
  data: z.any()
  // HUI data 会在这里
}).passthrough();
var HuiMcpClient = class extends Client {
  serverSupportsHui = null;
  discoveredHuiMetaToolName = null;
   ...
   */
  async listToolsWithHui(params, options) {
    const standardToolsResult = await super.listTools(params, options);
    if (!standardToolsResult || !standardToolsResult.tools) {
      return [];
    }
    let huiToolInfoArray = standardToolsResult.tools.map((tool) => ({
      ...tool,
      huiHints: void 0
    }));
    if (this.serverSupportsHui === null) {
      this.discoveredHuiMetaToolName = this.findHuiMetaTool(standardToolsResult.tools);
      ...
      try {
        ...
        if (sdkCallResult && sdkCallResult.content && Array.isArray(sdkCallResult.content)) {
          for (const contentItem of sdkCallResult.content) {
            ...
            if (genericJsonParse.success && genericJsonParse.data.data) {
        ...
        if (huiDefinitionsResult && huiDefinitionsResult.tools) {
          ...
          for (const toolWithHint of huiDefinitionsResult.tools) {
            hintsMap.set(toolWithHint.name, toolWithHint.huiHints);
          }
          huiToolInfoArray = huiToolInfoArray.map((tool) => ({
            ...tool,
  ...
  async getHuiHintsForTool(toolName, options) {
    if (this.serverSupportsHui === null) {
    ...
    if (this.serverSupportsHui && this.discoveredHuiMetaToolName) {
      try {
        ...
        const rawSdkResultSingleTool = await super.callTool(
          callParams,
          void 0,
          // Use default result schema
          options
        );
        const huiSpecificOutputSingleTool = rawSdkResultSingleTool.content;
        const result = HuiDefinitionsResultSchema.parse(huiSpecificOutputSingleTool);
        const toolHintData = result.tools?.find((t) => t.name === toolName);
        if (toolHintData) {
          return toolHintData.huiHints;
        }
      } catch (error) {
...
//# sourceMappingURL=hui-client.esm.js.map
